...
class HomePageSettingViewController: SettingsTableViewController, FeatureFlagsProtocol {
    ...
    var isRecentlyVisitedSectionEnabled: Bool {
        let isFeatureEnabled = featureFlags.isFeatureActiveForBuild(.recentlyVisited)
        let isNimbusFeatureEnabled = homescreen.sectionsEnabled[.recentlyVisited] == true
        guard isFeatureEnabled, isNimbusFeatureEnabled else { return false }
        return true
    }
    ...
    private func customizeFirefoxSettingSection() -> SettingSection {
        ...
        let librarySetting = BoolSetting(prefs: prefs, prefKey: PrefsKeys.LibraryShortcutsEnabled, defaultValue: true, attributedTitleText: NSAttributedString(string: .AppMenuLibraryTitleString))
        ...
        sectionItems.append(librarySetting)
    ...
    private func setupStartAtHomeSection() -> SettingSection? {
        ...
        let afterFourHoursOption = CheckmarkSetting(title: NSAttributedString(string: .Settings.Homepage.StartAtHome.AfterFourHours),
                                                    subtitle: nil,
