...
class NewDevicePickerViewController: UIViewController, DevicePicker {
    ...
    var dataSource: UICollectionViewDiffableDataSource<Int, RemoteDevice>!
...
extension NewDevicePickerViewController {
    ...
    private func configureDataSource() {
        let cellRegistration = UICollectionView.CellRegistration<DevicePickerCell, RemoteDevice> { (cell, indexPath, item) in
            cell.updateWithDevice(item)
        }
        
        let headerRegistration = UICollectionView.SupplementaryRegistration<UICollectionReusableView>(elementKind: "Header"){ (view, kind, indexPath) in
        ...
        dataSource = UICollectionViewDiffableDataSource<Int, RemoteDevice>(collectionView: collectionView) { (collectionView: UICollectionView, indexPath: IndexPath, item: RemoteDevice) -> UICollectionViewCell? in
...
extension NewDevicePickerViewController {
    ...
    private func loadList(animated: Bool = true) {
        ...
        RustFirefoxAccounts.startup(prefs: profile.prefs).uponQueue(.main) { [weak self] accountManager in
            ...
            var snapshot = NSDiffableDataSourceSnapshot<Int, RemoteDevice>()
