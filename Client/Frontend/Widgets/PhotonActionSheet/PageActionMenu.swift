...
extension PhotonActionSheetProtocol {
    ...
    func getTabMenuActions(tab: Tab,
                           ...
                           success: @escaping (String, ButtonToastAction) -> Void) -> [UIMenuElement] {
        let urlBar = tab.browserViewController?.urlBar ?? UIView()
        if tab.url?.isFileURL ?? false {
            let shareFile = UIAction(title: Strings.AppMenuSharePageTitleString) { action in
        ...
        let toggleActionTitle = defaultUAisDesktop ? Strings.AppMenuViewDesktopSiteTitleString : Strings.AppMenuViewMobileSiteTitleString
        let toggleDesktopSite = UIAction(title: toggleActionTitle, state: (tab.changedUserAgent ? .on : .off)) { _ in
            if let url = tab.url {
        ...
        let zoomIn = UIAction(title: "Zoom In", attributes: tab.pageZoom == 3.0 ? .disabled : []) { _ in
            tab.zoomIn()
        }
        
        let zoomOut = UIAction(title: "Zoom Out", attributes: tab.pageZoom == 0.5 ? .disabled : []) { _ in
            tab.zoomOut()
        }

        let addReadingList = UIAction(title: Strings.AppMenuAddToReadingListTitleString) { _ in
        ...
        let bookmarkPage = UIAction(title: Strings.AppMenuAddBookmarkTitleString) { _ in
        ...
        let removeBookmark = UIAction(title: Strings.AppMenuRemoveBookmarkTitleString) { _ in
            guard let url = tab.url?.displayURL else { return }
        ...
        let pinToTopSites = UIAction(title: Strings.PinTopsiteActionTitle) { _ in
        ...
        let removeTopSitesPin = UIAction(title: Strings.RemovePinTopsiteActionTitle) { _ in
            guard let url = tab.url?.displayURL, let sql = self.profile.history as? SQLiteHistory else { return }
        ...
        let sendToDevice = UIAction(title: Strings.SendToDeviceTitle) { _ in
            ...
            let devicePickerViewController = NewDevicePickerViewController()
        ...
        let sharePage = UIAction(title: Strings.AppMenuSharePageTitleString) { _ in
        ...
        let copyURL = UIAction(title: Strings.AppMenuCopyURLTitleString) { _ in
        ...
        let refreshPage = UIAction(title: Strings.ReloadPageTitle) { _ in
            self.tabManager.selectedTab?.reload()
        }
        
        let stopRefreshPage = UIAction(title: Strings.StopReloadPageTitle) { _ in
        ...
        var mainActions = [zoomOut, zoomIn, sharePage]
        ...
        if tab.mimeType != MIMEType.PDF {
            let findInPageAction = UIAction(title: Strings.AppMenuFindInPageTitleString) { _ in
                findInPage()
            }
            let reportSiteIssueAction = UIAction(title: Strings.AppMenuReportSiteIssueTitleString) { _ in
                reportSiteIssue()
            }
        ...
        if shouldShowNewTabButton && tab.readerModeAvailableOrActive {
            return [UIMenu(options: .displayInline, children: refreshActions), UIMenu(options: .displayInline, children: mainActions), UIMenu(options: .displayInline, children: commonActions)]
        } else {
            return [UIMenu(options: .displayInline, children: mainActions), UIMenu(options: .displayInline, children: commonActions)]
        }
    }
