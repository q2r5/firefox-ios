...
class LoginListTableViewCell: ThemedTableViewCell {
    ...
    lazy var breachAlertContainer: UIView = .build { view in
    ...
    private lazy var textStack: UIStackView = .build { [weak self] stack in
        ...
        [self.hostnameLabel, self.usernameLabel].forEach { stack.addArrangedSubview($0) }
        stack.axis = .vertical
        stack.spacing = 8
    ...
    private lazy var contentStack: UIStackView = .build { [weak self] stack in
        guard let self = self else { return }
        [self.textStack, self.breachAlertContainer].forEach { stack.addArrangedSubview($0) }
